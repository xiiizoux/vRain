...
foreach my $i (0..$cln-1) {
    my $tm = $itm - 50 - 4*rand(2)*2;
    my $bm = $ibm - 50 - 4*rand(2)*2;
    ...
    $image->Draw(primitive => 'line', points => "$p2, $p3", stroke => $bc2, strokewidth => 2);
    ...
    $image->Draw(primitive => 'line', points => "$l1, $l2", stroke => $bc3, strokewidth => 2);
    $image->Draw(primitive => 'line', points => "$l2, $l3", stroke => $bc3, strokewidth => 1);
    $image->Draw(primitive => 'line', points => "$l3, $l4", stroke => $bc3, strokewidth => 2);
    ...
    $image->Draw(primitive => 'line', points => "$l5, $l6", stroke => $bc3, strokewidth => 2);
    $image->Draw(primitive => 'line', points => "$l6, $l7", stroke => $bc3, strokewidth => 1);
    $image->Draw(primitive => 'line', points => "$l7, $l8", stroke => $bc3, strokewidth => 2);
    ...
    foreach (0..$rlc) {
        my $ci = 230+rand(25);
        my $rc = "rgb($ci,$ci,$ci)";
        my @rcs = ('rgb(255,255,255)', 'rgb(200,200,200)', 'rgb(220,220,220)', 'rgb(180,180,180)');
        my $rx = $hm+$cw*$i+$cw*0.05+$cw*0.9*rand(10)/10;
        my $r1 = $rx.','.($itm+100+($ih-$itm-$ibm-100)*rand(10)/10);
        my $r2 = $rx.','.($itm+100+($ih-$itm-$ibm-100)*rand(10)/10);
        $image->Draw(primitive => 'line', points => "$r1, $r2", fill => $rcs[int(rand(3))], strokewidth => rand(30)/10);
    }
}
